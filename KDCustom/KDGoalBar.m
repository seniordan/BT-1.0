...
-(void)layoutSubviews {
    ...
    if ([customText length] != 0) {
        ...
        [percentLabel setText:[NSString stringWithFormat:@"%.1fKg", percent]];
    } else {
    ...
    labelFrame.origin.x = frame.size.width / 2 - percentLabel.frame.size.width / 2 + 3;
    labelFrame.origin.y = frame.size.height / 2 - percentLabel.frame.size.height / 2 - 25;
...
-(void)setup {
    ...
    [percentLabel setFont:[UIFont fontWithName:@"Futura-CondensedMedium" size:50]];
    [percentLabel setTextColor:[UIColor colorWithRed:246/255.0 green:185/255.0 blue:20/255.0 alpha:1.0]];
...
-(void)delayedDraw:(NSNumber *)newPercentage {
    ...
    percentLayer.percent = perc;
    [self setNeedsLayout];
    [percentLayer setNeedsDisplay];
    
    [self moveThumbToPosition:perc * (2 * M_PI)];
...
- (void)setPercent:(float)percent animated:(BOOL)animated {
    if (animated) {
//        finalPercent = MIN(120, MAX(0, percent));
        float oldPercent = percentLayer.percent ;
